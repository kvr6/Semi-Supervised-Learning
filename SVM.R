...
for( itr in 1:10){
    ...
    for (i in 1:length(nldata)) {
		...
		logitmodel = glm(Class ~., data=ldata, family = "binomial")
        pred.nb=data.frame(predict(logitmodel, tsdata[,-5],type="response"))
        pred.nb$label=sapply(1:nrow(pred.nb), function(x) ifelse(pred.nb[x,][1]>=0.5, 1, 0))
        ...
        predfunc.nb <- function(m,d) {
            pl <- predict(m,d)
            data.frame(cl=pl,p=pl)
        }
        
        #SelfTrain
        nbST <- SelfTrain(Class ~ .,semi.sup.data,learner('glm',list(family='binomial')),'predfunc.nb',thrConf = 0.999, maxIts = 10, verbose = FALSE,percFull = 1)
    
        pred.nb=data.frame(predict(nbST, tsdata[,-5],type="response"))
        pred.nb$label=sapply(1:nrow(pred.nb), function(x) ifelse(pred.nb[x,][1]>=0.5, 1, 0))
